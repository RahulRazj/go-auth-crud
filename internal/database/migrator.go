...
func Migrate(ctx context.Context, logger *logger.LoggerService, cfg *config.Config) error {
	hostPort := net.JoinHostPort(cfg.Database.Host, strconv.Itoa(cfg.Database.Port))

	// No need to create a full connection pool for migration, a single connection is sufficient
	...
	if from == int32(len(m.Migrations)) {
		logger.Info("database schema up to date, version %d", len(m.Migrations))
	} else {
		logger.Info("migrated database schema, from %d to %d", from, len(m.Migrations))
	}
